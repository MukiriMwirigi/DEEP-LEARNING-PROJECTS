...
def compute_probabilities(X, theta, temp_parameter):
    ...
    dot_products = itemp * theta.dot(train_x.T)
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    selected_probabilities = np.choose(train_y, probabilities)
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    M = sparse.coo_matrix(([1] * num_examples, (train_y, range(num_examples))), shape = (num_labels, num_examples)).toarray()
    non_regularized_gradient = np.dot(M - probabilities, train_x)
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    assigned_labels = get_classification(train_x,theta,temp_parameter)
    return 1 - np.mean(np.remainder(assigned_labels, 3) == train_y)
